...
def average_rating(list) -> list:
    rate = 0
    average_rate = float()
    if len(list) != 0:
        for i in range(0, len(list)):
            rate = rate + list[i][1]
        average_rate = rate / len(list)
    return average_rate
...
def merge_sort_reverse(list, average_rate):
    # Проверяем разбит ли список на отдельные части.
    if len(list) < 2:
        return list
    # Находим средину списка.
    ...
    # Разбиваем список на две части.
    left = merge_sort_reverse(list[:middle], average_rate)
    right = merge_sort_reverse(list[middle:], average_rate)

    # Объединяем отсортированные части в одну.
    merged = merge_reverse(left, right, average_rate)
    return merged


def merge_reverse(left, right, average_rate):
    # Если левая или правая часть пуста, мы имеем дело список
    # единственной частью, которая уже отсортирована.
    if not len(left):
        return left
    ...
    while (len(result) < totalLen):
        ...
        if left[leftindex][1] > right[rightindex][1] and (left[leftindex][1] > average_rate or right[rightindex][1] > average_rate):
            result.append(left[leftindex])
            leftindex += 1
        else:
            result.append(right[rightindex])
...
if __name__ == '__main__':
    ...
    with open(input_file, 'r') as infile:
        ...
        hash_dict_sorted = dict(hash_dict_sorted_tuple)
        #  hash_dict = merge_sort(hash_dict)
        query_count = int(infile.readline())
        for _ in range(query_count):
            query = infile.readline()
            relevant_docs = calc_relevance(hash_dict_sorted, query)
            average_rate = average_rating(relevant_docs)
            relevant_docs_sorted = merge_sort_reverse(relevant_docs, average_rate)
            relevant_docs_sorted = merge_sort(relevant_docs_sorted)
